...
Page({
    ...
    onLoad: function onLoad() {
        ...
        _bus2.default.on('deleteBlogEvent', this.$OnDeleteBlogEvent).on('likeEvent', this.$OnLikeEvent);
    ...
    onUnload: function onUnload() {
        _bus2.default.off('deleteBlogEvent', this.$OnDeleteBlogEvent).off('likeEvent', this.$OnLikeEvent);
    },

    onShow: function onShow() {
        if (_dataStore2.default.get('hasNewBlog')) {
    ...
    onPullDownRefresh: function onPullDownRefresh() {
        ...
        this.fetchBlogRequestEvent().then(function (result) {
            ...
            _dataStore2.default.put('hasNewBlog', false);
    ...
    $OnDeleteBlogEvent: function $OnDeleteBlogEvent(id) {
        var blogArr = this.data.blogArr;

        var deleteItem = blogArr.find(function (item) {
            return item._id === id;
        });
        if (!deleteItem) return;

        var index = blogArr.indexOf(deleteItem);
    ...
    $OnLikeEvent: function $OnLikeEvent(id, hasLike) {
        ...
        var likeItem = blogArr.find(function (item) {
            return item._id === id;
        });
        if (!likeItem) return;
        var index = blogArr.indexOf(likeItem);
        var likeCount = this.data.blogArr[index].likeCount;

        var likeCountKey = 'blogArr[' + index + '].likeCount';
        var hasLikeKey = 'blogArr[' + index + '].hasLike';

        var newCount = hasLike ? likeCount - 1 : likeCount + 1;
        this.setData((_setData2 = {}, _defineProperty(_setData2, likeCountKey, newCount), _defineProperty(_setData2, hasLikeKey, !hasLike), _setData2));
    }
});
